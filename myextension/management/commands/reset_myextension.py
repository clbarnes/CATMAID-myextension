...
class Command(BaseCommand):
    help = 'Deletes all rows from database tables backing the myextension models.'
    ...
    def handle(self, *args, **options):
        ...
        while selection.lower() not in ['y', 'n', '']:
            selection = input('This will empty all myextension-related database tables. Are you sure ([y]/n)? ')
        ...
        for model in apps.get_app_config('myextension').get_models():
        ...
        self.stdout.write(self.style.SUCCESS('Successfully cleared myextension tables'))
